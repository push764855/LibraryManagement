...
public class ReturnBookController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String searchByID = request.getParameter("txtSearchByID");
            session.setAttribute("USER_ID_SEARCH", searchByID);

            OrderDetailDAO dao = new OrderDetailDAO();
            List<OderDetailDTO> list = dao.getListOrderByUserID(searchByID);
            if (list != null){
                session.setAttribute("LIST_ORDERED", list);
                url = SUCCESS;
            }else {
                request.setAttribute("RETURN_MSG", "No book is hiring !!! ");
            }
